...
export const definitions = {
  ...
  /* ── PeerReview.ai custom components ──────────────────────────────────────
   * Domain panels for the marking cockpit. The agent emits these via the
   * Python surface builders (deterministic), so layout is trusted app code. */

  CodeBlock: {
    description:
      "Monospace source-code block with optional title and highlighted lines. Use to show a student's submitted entry-point or an evidence snippet.",
    props: z.object({
      code: z.string(),
  ...
  ReferenceContextPanel: {
    description:
      "LinkUp source-grounded reference. Shows the topic, a short explanation, cited sources, and an explicit guardrail that the reference is explanation-only and does not affect scoring.",
    props: z.object({
      topic: z.string(),
      answer: z.string(),
      sources: z
      ...
      gradingImpact: z.string().optional(),
    }),
  },

  TestResultsPanel: {
    description:
      "Pass/fail results from the frozen test suite, with a summary count and per-test status + failure message.",
    props: z.object({
      summary: z.object({
        total: z.number(),
        passed: z.number(),
  ...
  MisconceptionPanel: {
    description:
      "Highlights a detected conceptual error (e.g. DFS instead of BFS), with severity, an explanation, and evidence bullets drawn from real test output and code.",
    props: z.object({
      label: z.string().optional(),
      title: z.string(),
  ...
  GradeApprovalPanel: {
    description:
      "Human-in-the-loop grade gate. Renders each rubric criterion with an editable score, a 'show failed tests to student' toggle, and an Approve button that dispatches approve_grades with the (possibly edited) scores.",
    props: z.object({
      criteria: z.array(
        z.object({
          id: z.string(),
